...
class SettingController extends Controller {
	...
	public function meta(Request $request, $id) {
		request()->validate([
			'home_page_content' => 'required|string',
			'featured_image' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:10240|dimensions:max_width=5000,max_height=3000',
		...
		$setting = Setting::find($id);
		$setting->home_page_content = $request->input('home_page_content');
		...
		if ($request->hasFile('featured_image')) {
			...
			$file_name = $this->featured_image($image);
			$setting->featured_image = $file_name;
		}
	...
	public function featured_image($image) {
		...
		$location = public_path('web/home-image/' . $filename);
		// create new image with transparent background color
