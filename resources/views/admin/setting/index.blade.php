			...
			<div class="nav-tabs-custom">
				...
				<div class="tab-content">
					...
					<div class="tab-pane" id="home_page">
						<form data-parsley-validate class="form-horizontal" action="{{ route('admin.settingMetaRoute', $setting->id) }}" method="post" enctype="multipart/form-data">
							{{csrf_field()}}

							<div class="form-group{{ $errors->has('home_page_content') ? ' has-error' : '' }}">
								<label for="home_page_content" class="col-sm-2 control-label">Home Page Content</label>
								<div class="col-sm-10">
									<textarea name="home_page_content" class="form-control summernote" id="home_page_content" required>{{ $setting->home_page_content }}</textarea>
									@if ($errors->has('home_page_content'))
									<span class="help-block">
										<strong>{{ $errors->first('home_page_content') }}</strong>
									</span>
									@endif
								</div>
							</div>
	...
	<script type="text/javascript">
		$(function(){
			$('.summernote').summernote({
				height: 200,
				placeholder: "ex: home page content"
			})
		});
	</script>
	<!-- /.editor -->
